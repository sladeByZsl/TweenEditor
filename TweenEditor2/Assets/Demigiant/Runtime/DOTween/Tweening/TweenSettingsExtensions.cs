...
namespace DG.Tweening
{
	public static class TweenSettingsExtensions
	{
		...
		public static TweenerCore<Vector3, Path, PathOptions> SetLookAt(this TweenerCore<Vector3, Path, PathOptions> t, Transform lookAtTransform, Vector3? forwardDirection, Vector3? up)
		{
			if ((t != null) && t.active)
			{
		...
		public static T SetLoops<T>(this T t, int loops) where T: Tween
		{
			if (((t != null) && t.active) && !t.creationLocked)
			{
				...
				if (loops > -1)
				{
					t.fullDuration = t.duration * loops;
					return t;
				}
				t.fullDuration = float.PositiveInfinity;
			}
		...
		private static void SetPathForwardDirection(this TweenerCore<Vector3, Path, PathOptions> t, Vector3? forwardDirection, Vector3? up)
		{
			Vector3? nullable;
			Vector3 zero;
			if ((t == null) || !t.active)
			...
			if (forwardDirection.HasValue)
			{
				...
				zero = Vector3.zero;
				if (nullable.HasValue ? (nullable.HasValue ? (nullable.GetValueOrDefault() != zero) : false) : true)
				{
					goto Label_0085;
				}
			}
			...
			Label_0085:;
			t.plugOptions.hasCustomForwardDirection = true;
